(() => {
  ...
  function playerRow(p){return `<div class="matchup-player-row"><span class="pos">${p.position||'—'}</span><span class="matchup-player-name">${p.full_name||p.player_id}<span class="matchup-player-meta">${[p.team,p.injury_status].filter(Boolean).join(' · ')}</span></span><span class="matchup-player-num">${p.projection!=null?Number(p.projection).toFixed(2):'—'}</span><span class="matchup-player-num">${pts(p.player_id).toFixed(2)}</span></div>`;}
  function lineup(team){if(!team)return'<div class="muted">Roster unavailable.</div>';const starterIds=new Set((team.starters||[]).map(p=>String(p.player_id)));const starters=(team.starters||[]);const bench=(team.players||[]).filter(p=>!starterIds.has(String(p.player_id))&&!p.reserve&&!p.taxi);return `<div class="matchup-lineup"><div class="matchup-lineup-title"><span>${team.team_name||team.display_name||'Team'}</span><span>LINEUP</span></div><div class="matchup-player-head"><span>POS</span><span>PLAYER</span><span style="text-align:right">PROJ</span><span style="text-align:right">PTS</span></div>${starters.map(playerRow).join('')||'<div class="muted">No starters.</div>'}${bench.length?`<div class="matchup-bench-label">Bench</div>${bench.map(playerRow).join('')}`:''}</div>`;}
  function matchupCard(group){const a=group[0],b=group[1]||null;const ta=teamByRoster(a?.roster_id),tb=teamByRoster(b?.roster_id);const aName=ta?.team_name||ta?.display_name||'Team',bName=tb?.team_name||tb?.display_name||(b?'Team':'Bye');const aOwner=ta?.username||ta?.display_name||'',bOwner=tb?.username||tb?.display_name||'';const mine=[String(a?.roster_id),String(b?.roster_id)].includes(mineId());const aPts=rosterActual(a),bPts=b?rosterActual(b):0;const aProj=starterProjection(ta),bProj=tb?starterProjection(tb):0;return `<details class="panel matchup-card ${mine?'mine':''}"><summary><div class="matchup-summary"><div class="matchup-team"><span class="matchup-team-name">${aName}</span><span class="matchup-team-owner">${aOwner?`@${aOwner}`:''}</span></div><div class="matchup-score"><strong>${aPts.toFixed(2)}</strong><span class="matchup-vs">${b?'VS':'BYE'}</span><strong>${b?bPts.toFixed(2):'—'}</strong></div><div class="matchup-team away"><span class="matchup-team-name">${bName}</span><span class="matchup-team-owner">${bOwner?`@${bOwner}`:''}</span></div></div><div class="matchup-proj-row"><div><span>${aName}</span><strong>${aProj.toFixed(2)} proj</strong></div><div><span>${bName}</span><strong>${b?`${bProj.toFixed(2)} proj`:'—'}</strong></div></div><span class="matchup-expand-hint">Tap to view players</span></summary><div class="matchup-detail"><div class="matchup-lineups">${lineup(ta)}${b?lineup(tb):''}</div></div></details>`;}
